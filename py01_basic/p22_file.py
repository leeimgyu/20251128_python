...
import collections
r = open('test.txt', mode='rt', encoding='utf-8')
# 파일 객체 r이 반복 가능한 객체인지 확인
is_iterable = isinstance(r, collections.abc.Iterable)
print(f"r은 Iterable한 가요? {is_iterable}")

# 파일 객체 r이 TextIOWrapper 클래스의 인스턴스인지 확인
# TextIOWrapper:: 바이트스트림을 문자열스트림으로 변환하는 객체(네트워큭전송시 활용)
is_text_wrapper = isinstance(r, io.TextIOWrapper)
print(f"r은 TextIOWrapper한 가요? {is_text_wrapper}")
r.close()

# try:
#     f = open('test.txt', mode='wt', encoding='utf-8')
#     f.write('1파이썬으로 파일을 작성하고 있습니다.')
#     f.write('newline 문자로 개행해봅니다.\n')
#     f.write('개행이 잘되었나요?')
# finally:
#     f.close()
